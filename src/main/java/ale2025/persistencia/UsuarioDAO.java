...
import ale2025.dominio.Usuario;        // Clase que representa la entidad de usuario en el dominio de la aplicación.
import ale2025.utils.PasswordHasher; // Clase utilitaria para el manejo seguro de contraseñas (hash, verificación).
...
public class UsuarioDAO {
     ...
     *
     * @param usuario El objeto User que contiene la información del nuevo usuario a crear.
    ...
    public Usuario create(Usuario usuario) throws SQLException {
        Usuario res = null; // Variable para almacenar el usuario creado que se retornará.
        try{
            ...
            ps.setString(1, usuario.getNombre()); // Asignar el nombre del usuario.
     ...
     *
     * @param usuario El objeto User que contiene la información actualizada del usuario.
    ...
    public boolean update(Usuario usuario) throws SQLException{
        ...
        try{
            ...
            ps = conn.connect().prepareStatement(
                    "UPDATE Usuario " +
                            "SET nombre = ?, correoElectronico = ?, estado = ? " +
                            "WHERE id = ?"
            );
            // Establecer los valores de los parámetros en la sentencia preparada.
            ps.setString(1, usuario.getNombre());  // Asignar el nuevo nombre del usuario.
            ps.setString(2, usuario.getCorreoElectronico()); // Asignar el nuevo correo electrónico del usuario.
            ps.setByte(3, usuario.getEstado());    // Asignar el nuevo estado del usuario.
            ps.setInt(4, usuario.getId());       // Establecer la condición WHERE para identificar el usuario a actualizar por su ID.
     ...
     *
     * @param usuario El objeto User que contiene el ID del usuario a eliminar.
    ...
    public ArrayList<Usuario> search(String name) throws SQLException{
        ...
        try {
            ...
            while (rs.next()){
                ...
                usuario.setId(rs.getInt(1));       // Obtener el ID del usuario.
                usuario.setNombre(rs.getString(2));   // Obtener el nombre del usuario.
                usuario.setCorreoElectronico(rs.getString(3));  // Obtener el correo electrónico del usuario.
                usuario.setEstado(rs.getByte(4));    // Obtener el estado del usuario.
    ...
    public Usuario getById(int id) throws SQLException{
        ...
        return usuario; // Retornar el objeto User encontrado o null si no existe.
    }
    /**
     ...
     *
     * @param usuario El objeto User que contiene el correo electrónico y la contraseña
     ...
     *
     * @param usuario El objeto User que contiene el ID del usuario cuya contraseña se
